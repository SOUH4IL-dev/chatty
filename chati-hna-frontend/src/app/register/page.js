...
export default function RegisterPage() {
  ...
  const setupProfileAndRedirect = async (user, displayName) => {
    const { data } = await api.put('/auth/me', { name: displayName || name });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await setupProfileAndRedirect(result.user, name);
    } catch (err) {
      switch (err.code) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      await setupProfileAndRedirect(result.user, displayName);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background text-foreground p-4">
      ...
      >
        ...
        <p className="text-gray-400 text-center mb-8">Join the conversation today</p>
